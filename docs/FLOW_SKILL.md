...
Flow 支持两种添加参考图/角色的方式，适用于不同场景。

### 模式对比

| 特性 | Character 模式（@ 方式）⭐ | Upload 模式（加号方式） |
|------|--------------------------|----------------------|
| **入口** | 输入框输入 `@` | 点击加号按钮 (230, 327) |
| **适用场景** | 已有的 Character 角色 | 上传的图片文件 |
| **包含信息** | 角色信息 + 声音 + 外观 | 仅图片 |
| **视频生成** | ✅ 推荐（包含声音） | ⚠️ 仅外观参考 |
| **图片生成** | ✅ 推荐 | ✅ 可用 |
| **稳定性** | ✅ 高 | ⚠️ 第二张可能失败 |
| **推荐程度** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ |

### 模式 1: Character 模式（@ 方式）⭐ 推荐

**何时使用：**
- ✅ 已经创建了 Character 角色
- ✅ 需要生成视频（Character 包含声音信息）
- ✅ 需要稳定的角色一致性
- ✅ 多角色场景
...
步骤 1: 点击输入框
步骤 2: 输入 @ → 触发搜索对话框
步骤 3: 点击 "Characters" 标签页（role="tab"，不是侧边栏）
步骤 4: 点击角色（如 luna）
步骤 5: 点击 "Add to Prompt" 按钮
步骤 6: ✅ 角色添加到输入框（显示 "luna"）
步骤 7: 重复步骤 2-6 添加其他角色
```

**⚠️ 关键细节：**
- 必须点击 `role="tab"` 的 **Characters 标签页**（在弹出对话框的顶部）
- 不是点击左侧边栏的 Characters 按钮！
- 点击角色后必须点击 "Add to Prompt" 按钮
- 添加后对话框会自动关闭
- 添加下一个角色需要重新输入 `@`

**验证方法：**
```
✅ 正确：输入框显示 "luna" 和 "wayne"（只有角色名）
❌ 错误：输入框显示漫画标题或其他内容
```
...
# 角色选项
character = page.locator('[role="option"]').filter(has_text="luna").first
...
```
步骤 1: 点击输入框旁边的加号按钮（坐标 230, 327）
步骤 2: 等待媒体选择器弹出
步骤 3: 点击 "Uploads" 分类
步骤 4: 点击图片文件名（如 Luna_on_mars.png）
步骤 5: 点击 "Add to Prompt"
步骤 6: ⚠️ 重复步骤 1-5 添加其他图片（第二张可能失败）
```
...
**何时选择这个模式：**
- 图片在 Uploads 中，还未创建为 Character
- 只需要外观参考
- 不需要生成视频

### 选择建议

**✅ 优先使用 Character 模式（@ 方式）：**
1. 如果需要生成视频 → 必须用 Character（包含声音）
2. 如果已经创建了 Character → 用 Character
3. 如果需要角色一致性 → 用 Character
4. 如果需要添加多个角色 → 用 Character（更稳定）

**⚠️ 使用 Upload 模式（加号方式）：**
1. 仅当图片未创建为 Character 时
2. 仅用于图片生成（不生成视频）
3. 作为备选方案

**🎯 最佳实践：**
1. **将常用的角色创建为 Character** ← 重要！
2. 优先使用 Character 模式（@ 方式）
3. Upload 模式仅作为临时方案
4. 视频生成必须用 Character 模式
